...
test("createProject writes a project.plexus manifest and returns a project", async () => {
    ...
    assert.equal((project as unknown as TodlPackageProject).Name, "My Package");
...
test("openProject reads the name back from the manifest", async () => {
    ...
    assert.equal((project as unknown as TodlPackageProject).Name, "Reopened");
});
...
test("saveProject round-trips through storage", async () => {
    ...
    assert.equal((reopened as unknown as TodlPackageProject).Name, "Round");
});
