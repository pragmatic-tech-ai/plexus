...
resources SolutionResources {
    ...
    // Save), the cross-project CONNECTION picker docked at the bottom (shown only
    // when a solution is open), and the member/folder tree filling the rest.
    DataTemplate [DataType = SolutionExplorerService] {
        DockPanel [ LastChildFill = true ] {
            ...
            // The cross-project connection picker — docked at the top (below the
            // toolbar) so its dropdown opens into the pane, not off the window edge.
