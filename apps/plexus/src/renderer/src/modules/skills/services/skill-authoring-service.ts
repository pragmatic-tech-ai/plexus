...
import { type IEnvironment } from '@pragmatic-tech-ai/todl-runtime';
...
export class SkillAuthoringService extends ServiceBase {
    ...
    constructor(provider: IServiceProvider, deps?: AuthoringDeps) {
        ...
        this._save = new RelayCommand(
            () => {
                void this._session?.save();
            },
            () => this.canSave(),
        );
        ...
        if (deps === undefined)
            this.Provider.get(OpenProjectsStore.Key)?.Subscribe(() => {
                void this.Refresh();
            });
    }
    ...
    get Skills(): readonly Skill[] {
        ...
        if (q === '') return all;
        return all.filter((s) =>
            [s.Name, s.Title, s.Description].some((f) => f.toLowerCase().includes(q)),
        );
    }
    ...
    private buildDeps(): AuthoringDeps {
        return {
            ...
            scaffold: (req) =>
                new SkillScaffolder(this.scaffoldFs(), {
                    projectDir: req.projectDir ?? this.projectDirs()[0],
                    homeDir: this.env().HomeDirectory,
                }).create(req),
    ...
    private async presentNewDialog(defaultScope: SkillScope): Promise<NewSkillRequest | undefined> {
        ...
        const result = await dialogs.Show<NewSkillRequest | undefined>({
            Title: 'New Skill',
            Content: dialog,
            Width: 440,
        });
    ...
    private scaffoldFs(): ScaffoldFs {
        ...
        return {
            ...
            createDirectory: (p) => fs.CreateDirectory(p),
