...
// LocalFileStorage — the disk-backed IStorage. Rooted at an absolute OS folder;
...
// join against the (possibly `\`-separated) root using the same separator the root
// uses, so Windows and POSIX roots both work without node:path.
export class LocalFileStorage implements IStorage, ILocalFileAccess
{
    ...
    // to a fresh <id>/<version>/ path just works. A top-level write (no parent
    // segment) relies on the root already existing and skips the mkdir.
