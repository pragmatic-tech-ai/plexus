...
// file-system capability the main process owns. The shared IO seam: VMs /
// commands / storage backends resolve it (FileSystemService.Key) and call
// PascalCase methods; they never touch `window.api`, ipcRenderer, or node:fs, so
...
// cancels. Pair with LocalFileStorage for a rooted IStorage over it.
    ...
    constructor(provider: IServiceProvider)
    {
        ...
        if (bridge?.fs === undefined)
        {
            throw new Error(
                ...
                + 'bridge did not load (expose it with createFileSystemBridge). This '
                + 'service requires a desktop host.',
            );
        }
        this.api = bridge.fs;
    }
